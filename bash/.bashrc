...
# slightly increse the limit to reduce the number of such tmp files
HISTSIZE=1000
HISTFILESIZE=1000
...
git_default_branch() {
    # handle "master" or "main" depending on remote repo settings
    git symbolic-ref refs/remotes/origin/HEAD | cut -d"/" -f4
}
alias gm='git switch $(git_default_branch)'
alias gp="git pull"
alias gmgp="gm && gp"
# diffs
git_diff() {
    ...
    if [ "$#" -eq 0 ]; then
        $cmd
    elif [ "$1" = "--name-only" ]; then
        $cmd --name-only | treefile
    fi
}
gd() {
    git_diff "git diff" "$@"
...
gdm() {
    git_diff "git diff $(git_default_branch)..." "$@"
}
complete -W "--name-only" gd gdm
